          ...
          body: Padding(
            ...
            child: NestedScrollView(
              headerSliverBuilder: (context, innerBoxIsScrolled) {
                return [
                  SliverAppBarWidget(
                        title: 'Choose Time of Task',
                        icon: Icons.arrow_back,
                        iconSize: 30,
                        onPressed: () {
                          Navigator.pop(context);
                        },
                        backgroundColor: state is OnDateRadioButtonsSelected &&
                                    cubit.onDateSelected != null ||
                                state is BeforeDateRadioButtonsSelected &&
                                    cubit.beforeDateSelected != null ||
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      DateCustomRadioButtons(
                        index: 1,
                        title: state is OnDateRadioButtonsSelected &&
                                state.onDateSelected != null
                            ? 'On ${DateFormat.yMMMMd().format(DateTime(state.onDateSelected!.year, state.onDateSelected!.month, state.onDateSelected!.day))}'
                            : 'On Date',
                        onPressed: () {
                          DateRadioButtonCubit.get(context)
                          ...
                          showDialog(
                              ...
                              builder: (context) {
                                return CalenderDialog(
                                  cubit: cubit,
                                  onSubmit: (p0) {
                                    cubit.OnDateSelected(
                                        index: 1, date: p0 as DateTime);
                                    Navigator.pop(context);
                                  },
                      ...
                      DateCustomRadioButtons(
                        index: 2,
                        title: state is BeforeDateRadioButtonsSelected &&
                                state.beforeDateSelected != null
                            ? 'Before ${DateFormat.yMMMMd().format(DateTime(state.beforeDateSelected!.year, state.beforeDateSelected!.month, state.beforeDateSelected!.day))}'
                            : 'Before Date',
                        onPressed: () {
                          DateRadioButtonCubit.get(context)
                              .BeforeDateSelected(index: 2);
                          showDialog(
                              context: context,
                              builder: (context) {
                                return CalenderDialog(
                                  cubit: cubit,
                                  onSubmit: (p0) {
                                    cubit.BeforeDateSelected(
                                        index: 2, date: p0 as DateTime);
                                    Navigator.pop(context);
                                  },
                  ...
                  onPressed: () {
                    state is OnDateRadioButtonsSelected &&
                            ...
                            state is BeforeDateRadioButtonsSelected &&
                                cubit.beforeDateSelected != null ||
                            state is FlexibleDateRadioButtonsSelected
                        ? Navigator.pushNamed(context, TaskPlace.id)
                        : null;
                  },
                ),
        );
      ...
      child: Container(
        ...
        child: OutlinedButton(
          ...
          child: Text(
            title,
            style: TextStyle(
              color: state is OnDateRadioButtonsSelected &&
